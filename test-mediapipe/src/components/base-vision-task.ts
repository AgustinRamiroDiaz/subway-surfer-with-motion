...
// Modified for webcam-motion-games: adds opt-in per-frame profiling metadata.

import { ViewToggle } from './view-toggle';
import { BaseTask, BaseTaskOptions } from './base-task';
import { recordCapture, recordResult } from '../performance-profile';
...
export abstract class BaseVisionTask extends BaseTask {
  ...
  protected profileFrameSequence = 0;
  ...
  protected override handleWorkerMessage(event: MessageEvent) {
    ...
    switch (type) {
      case 'DETECT_RESULT':
        const { mode, result, inferenceTime, frameId, handCount } = event.data;
        const workerResultAtMs = performance.now();
        ...
        if (mode === 'IMAGE') {
          ...
          recordResult(frameId, workerResultAtMs, performance.now(), inferenceTime, handCount);
  ...
  protected async predictWebcam() {
    ...
    if (this.video.currentTime !== this.lastVideoTimeSeconds) {
      ...
      try {
        ...
        const captureDoneAtMs = now;
        this.profileFrameSequence += 1;
        const frameId = this.profileFrameSequence;
        recordCapture(frameId, captureStartedAtMs, captureDoneAtMs);
